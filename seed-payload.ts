...
async function seed() {
  ...
  try {
    ...
    const [adminUser] = await db.insert(users).values({
      ...
      salt: hashedPassword.split('$')[2], // Extract salt from bcrypt hash
      hash: hashedPassword, // Full bcrypt hash
      firstName: "Admin",
      lastName: "User",
      authProvider: "email",
      emailVerified: true,
    ...
    await db.insert(membershipPlans).values({
      ...
      price: "1500.00",
      duration: 30,
      features: JSON.stringify(["Gym access 6am-10pm", "1 free class per week", "Locker access"]),
      popular: false,
      active: true
    });
    ...
    const [premiumPlan] = await db.insert(membershipPlans).values({
      ...
      price: "3000.00",
      duration: 30,
      features: JSON.stringify(["24/7 gym access", "Unlimited classes", "Personal locker", "1 personal training session/month"]),
      popular: true,
      active: true
    }).returning();
    ...
    const [vipPlan] = await db.insert(membershipPlans).values({
      ...
      price: "5000.00",
      duration: 30,
      features: JSON.stringify(["24/7 gym access", "Unlimited classes", "Premium locker", "3 personal training sessions/month", "Access to VIP lounge"]),
      popular: false,
      active: true
    }).returning();
